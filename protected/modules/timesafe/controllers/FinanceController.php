...
class FinanceController extends RController
{
    ...
    public function actionList() {
        ...
        if (isset($_GET['Finance'])) {
            ...
            Yii::app()->user->setState('_filter_Finance', $_GET['Finance']);
        } else
            if (Yii::app()->user->hasState('_filter_Finance')) {
                $model->attributes = Yii::app()->user->getState('_filter_Finance');
            }
    ...
    public function actionUpdate($id) {
        ...
        $this->performAjaxValidation($model);
        $page = (int)Yii::app()->request->getParam('Finance_page');
    ...
    public function actionDelete($id) {
       //$id = Yii::app()->request->getParam('id');
			...
			$model = Finance::model()->find("id = '".$id."'");
			$model->delete();
            if (!isset($_GET['ajax'])) {
    ...
    public function beforeAction($action) {
        if ($_GET['ajax'] === 'state')
            if (count($_GET['FinanceCheck']) > 0) {
                foreach ($_GET['FinanceCheck'] as $column => $value) {
                    foreach ($value as $id => $val) {
                        Finance::model()->updateByPk((int)$id, array($column => (int)$val));
                    }
                }
                Yii::app()->end();
            }
